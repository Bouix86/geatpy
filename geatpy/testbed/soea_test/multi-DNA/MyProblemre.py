    ...
    def aimFunc(self, pop):  # 目标函数
        ...
        for i in range(X.shape[0]):
            for j in range(X.shape[1] - 2):
                if (X[i][j + 2] == 0):
                    T_local = C / F_localCPU# 100
                    E_local = C * e_localCPU#100
                    infor[i][j] = 0.5 * T_local + 0.5 * E_local#200
                elif (X[i][j+2] == 1):
                    T_cloud = C / F_cloudCPU + D / R_cellular# 2+10
                    E_cloud = D * e_cloudcellular# 186
                    infor[i][j] = 0.5 * T_cloud + 0.5 * E_cloud#206
                else:
                    T_MEC = C / F_MECCPU + D / R_wifi#20+2
                    E_MEC = C * e_MECCPU + D * e_MECwifi#50+126
                    infor[i][j] = 0.5 * T_MEC + 0.5 * E_MEC#198
