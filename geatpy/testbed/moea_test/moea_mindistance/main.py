...
if __name__ == '__main__':
    ...
    plt.scatter(NDSet.Phen[:, 0].T, NDSet.Phen[:, 1].T, s=10, c='b')
    # problem.places = np.delete(problem.places, 10, axis=0)
    # plt.text(0, 0, "Total distance=%.5f" % best_ObjV, fontdict={'size': 20, 'color': 'red'})
    plt.xlim((0, 1.0))
    plt.ylim((0, 1.0))
    # plt.grid(True)
    plt.xlabel('x坐标')
