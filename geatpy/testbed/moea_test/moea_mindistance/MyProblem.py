...
class MyProblem(ea.Problem):  # 继承Problem父类
    ...
    def aimFunc(self, pop):  # 目标函数
        ...
        for i in range(x.shape[0]):
            distance = np.sum(
                np.sqrt(np.square(self.places[:, 0] - x[i][0]) + np.square(self.places[:, 1] - x[i][1])))  # 计算总路程
            f1.append(distance)
        f2 = []  # 存储所有种群个体对应的总路程
        for i in range(x.shape[0]):
            distance = np.sum(
                np.sqrt(np.square(self.places[:, 0] - x[i][0]) + np.square(self.places[:, 1] - x[i][1])) * self.places[:,0])  # 计算总路程
            f2.append(distance)
        pop.ObjV = np.vstack([np.array([f1]), np.array([f2])]).T  # 把求得的目标函数值赋值给种群pop的ObjV
